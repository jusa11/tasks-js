...
// мой варинт. Немного неправильно, потому-что мутирую исходный объект
/* const isElementInArray = (searchElement, inputArray) => {
	if (typeof searchElement !== 'object') {
		return inputArray.includes(searchElement)
	}

	let searchElementToStr = JSON.stringify(searchElement)

	if (typeof searchElement === 'object') {
		return JSON.stringify(inputArray).includes(searchElementToStr)
	}
} */

// вариант их решения, через метод map возвращается новый массив и в нем уже ищется искомый элемент
const isElementInArray = (searchElement, inputArray) => {
	if (typeof searchElement !== 'object') {
		return inputArray.includes(searchElement)
	}
